  ...
  const bodyFat = useMemo(() => {
    ...
    if (gender === "male") {
      return +(
        495 /
          (1.0324 -
            0.19077 * safeLog10(waist - neck) +
            0.15456 * safeLog10(height)) -
        450
      ).toFixed(1);
    }
    return +(
      495 /
        (1.29579 -
          0.35004 * safeLog10(waist + hip - neck) +
          0.221 * safeLog10(height)) -
      450
    ).toFixed(1);
      ...
      <div className="max-w-6xl mx-auto space-y-8 sm:space-y-12">
        ...
        >
          ...
          <p className="text-gray-400 max-w-3xl text-base sm:text-lg">
            Track vitals instantly with responsive sliders, live feedback, and
            smart insights. No data is stored — perfect for quick assessments
            before you book a free consult with our coaches.
          </p>
        </motion.div>
          ...
          >
            <div className="flex items-center justify-between mb-6 gap-3">
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Body Mass Index with instant interpretation.
                </p>
              </div>
              <div
                className={`px-4 py-2 rounded-full text-sm font-semibold ${bmiBadgeColor(
                  bmiValue
                )}`}
              >
                {bmiLabel(bmiValue)}
                ...
                <div key={field}>
                  <div className="flex justify-between text-gray-400 text-sm mb-2">
                    ...
                    <span className="text-white font-semibold">
                      {bmiInputs[field]} {field === "height" ? "cm" : "kg"}
                    </span>
                  </div>
                  <input
                    type="range"
            ...
            <div className="mt-8 bg-primary-darker rounded-2xl p-5 border border-primary-lightGray/60">
              ...
              <p className="text-gray-300 text-sm mt-3">
                Keep BMI between 18.5 - 24.9 for optimal health. Our coaches use
                this score as a baseline and personalize the rest.
              </p>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  Navy formula body-fat estimate + calorie planner.
                </p>
              </div>
              <select
                value={bodyInputs.gender}
                onChange={(e) =>
                  setBodyInputs({ ...bodyInputs, gender: e.target.value })
                }
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {["waist", "neck", "hip"].map((field) => {
                const disabled =
                  field === "hip" && bodyInputs.gender === "male";
                return (
            ...
            <div className="bg-primary-darker rounded-2xl p-5 border border-primary-lightGray flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-4xl text-white font-black mt-1">
                  {isFinite(bodyFat) ? bodyFat : "—"}%
                </p>
              </div>
              <div className="text-sm text-gray-400">
                <p>Fitness Range: Men 10-20% | Women 18-28%</p>
                <p className="text-primary-red">
                  We build nutrition blocks to hit your target.
                </p>
              </div>
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setTdeeInputs({ ...tdeeInputs, activity: e.target.value })
                    }
                    className="w-full bg-primary-darker border border-primary-lightGray rounded-xl px-4 py-2.5 text-white focus:border-primary-red outline-none"
                  >
          ...
          <section className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {workoutPreviews.slice(0, 2).map((plan) => (
                <div
                  key={plan._id}
                  className="bg-primary-gray/60 border border-primary-lightGray rounded-2xl p-4"
                >
                  ...
                  <p className="text-gray-500 text-xs mt-2">
                    {plan.totalExercises} drills • locked
                  </p>
                </div>
              ))}
              {dietPreviews.slice(0, 2).map((diet) => (
                <div
                  key={diet._id || diet.title}
                  className="bg-primary-gray/60 border border-primary-lightGray rounded-2xl p-4"
                >
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-400 text-sm">
              Book a complimentary InBody scan at Elite Gym HQ. We combine
              DEXA-grade scans, posture analysis, and mobility screening to
              build the perfect workout & diet stack for you.
            </p>
          </div>
          <a
            href="/contact"
